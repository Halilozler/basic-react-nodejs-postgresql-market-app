...
const getUser = (req, res) => {
    global.db.query(dbCommand.getUserInformation + req.params.id).then((user) => {
        global.db.query(dbCommand.getAllBasket + req.params.id).then((basket) => {
...
const addMoney = (req, res) => {
    global.db.query(dbCommand.moneyIncrement, [req.params.money, req.params.id]).then((user) => {
        getUser(req, res);
    }).catch((err) => {
...
const removeMoney = (req, res) => {
    global.db.query(dbCommand.moneyDecrement, [req.params.money, req.params.id]).then((user) => {
        getUser(req, res);
    }).catch((err) => {
...
const logUp = (req, res) => {
    ...
    global.db.query(dbCommand.logUpUser, userInformation).then(() => {
        global.db.query(dbCommand.logInUser, [req.body.username, req.body.password]).then(user => {
...
const getItem = (req, res) => {
    global.db.query(dbCommand.getItem).then((item) => {
...
const buyItem = async(req, res) => {
    ...
    try {
        ...
        await global.db.query(dbCommand.buyItem, [info.item.rows[0].price, info.user.rows[0].id])
    } catch (error) {
        return res.status(400).json({message:"Ürün Alınamadı", error})
    }
    await global.db.query(dbCommand.getBasket, array).then((basket) => {
        if(basket.rows.length == 0){
            //hiç sepete oluşturulmadıysa
            global.db.query(dbCommand.creatBasket, array).catch((error) => {
                return res.status(400).json({message:"Sepet oluşturulurken hata oluştu", error})
            })
        }else{
            //sepete varsa update yapsın
            global.db.query(dbCommand.setIncrementBasket, array).catch((error) => {
...
const sellItem = async(req, res) => {
    try {
        ...
        await global.db.query(dbCommand.sellItem, [info.item.rows[0].price, info.user.rows[0].id])
